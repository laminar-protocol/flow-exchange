...
export type Subscribe<T extends State> = <U>(subscriber: Subscriber<T, U>) => () => void;
...
export function create<TState extends State>(
...
): [UseStore<TState>, StoreApi<TState>, UseSelector<TState>] {
  let state: TState;
  ...
  const subscribe: Subscribe<TState> = <StateSlice>(subscriber: Subscriber<TState, StateSlice>) => {
    function listener() {
  ...
  const useStore: UseStore<TState> = <StateSlice>(
    ...
    return hasNewStateSlice ? (newStateSlice as StateSlice) : subscriber.currentSlice;
  };
