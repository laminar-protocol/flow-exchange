...
const RenderDeposit: React.FC<RenderDepositProps> = ({ visible, handleCancel, handleOk, poolId }) => {
  ...
  const onSubmit = async () => {
    if (currentAccount && api && amount) {
      return notificationHelper(
        api.depositLiquidity(currentAccount.address, poolId, toPrecision(amount, baseToken.precision)),
      ).then(() => {
        handleOk();
        setAmount(0);
      });
